...
# The emails in the 'train' folder is used to train your 
...
# punctuation is separated by a space or newline. 
# =============================================================
...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        def get_counts(filenames:list):
            ...
            which we're in now.
            """
            # Going through each file in filenames
            for file in filenames:
                # Creating a list of words that have been seen in this file
                # already to prevent duplicates
                words_seen = []
                # Getting the set of all the unique words in the file
                words_set = self.word_set(file)
                while len(words_set) > 0:
                    temp_word = words_set.pop()
                    if len(temp_word) > 1:
                        # Removing punctuation from current word
                        for char in temp_word:
                            if char in string.punctuation:
                                temp_word = temp_word.replace(char, "")
                    # Making current word all lowercase
                    temp_word = temp_word.lower()
                    if temp_word not in words_seen:
                        # Updating words_seen list
                        words_seen.append(temp_word)
                        # Updating word_counts_spam Dictionary
                        if filenames == train_spams:
                            if temp_word in self.word_counts_spam:
                                self.word_counts_spam[temp_word] = self.word_counts_spam[temp_word] + 1
                            else:
                                self.word_counts_spam[temp_word] = 1
                        # Updating word_counts_ham Dictionary
                        elif filenames == train_hams:
                            if temp_word in self.word_counts_ham:
                                self.word_counts_ham[temp_word] = self.word_counts_ham[temp_word] + 1
                            else:
                                self.word_counts_ham[temp_word] = 1

        # Getting the number of ham emails given
        self.num_train_hams = len(train_hams)

        # Getting the number of spam emails given
        self.num_train_spams = len(train_spams)

        # Making a DICTIONARY where word_counts_spam[word]
        # is the number of spam emails which contained this word.
        get_counts(train_spams)

        # Making a DICTIONARY where word_counts_ham[word]
        # is the number of ham emails which contained this word.
    ...
    def predict(self, filename:str):
        ...
        # Getting the set of all the unique words in the file passed in
        words_set = self.word_set(filename)

        # List of probabilities for spam words
        p_word_spam = []
        # List of probabilities for ham words
        p_word_ham = []

        # Adding P(spam) to p_word_spam list
        p_word_spam.append(p_spam)
        # Adding P(ham) to p_word_ham list
        p_word_ham.append(p_ham)
        ...
        while len(words_set) > 0:
            ...
            if temp_word not in words_seen:
                # Updating words_seen list
                words_seen.append(temp_word)
                # Updating p_word_spam and p_word_ham lists with the probability of the current word
                # after using Laplace Smoothing
                p_word_spam.append((self.word_counts_spam.get(temp_word, 0) + 1) / (self.num_train_spams + 2))
                p_word_ham.append((self.word_counts_ham.get(temp_word, 0) + 1) / (self.num_train_hams + 2))
        ...
        numerator = 0;
        for spam_probability in p_word_spam:
            numerator = numerator + spam_probability;

        # Calculating denominator
        denominator = numerator
        for ham_probability in p_word_ham:
            denominator = denominator + ham_probability

        # Returning the prediction of my Naive Bayes classifier
